...
class DSBackgroundPainter extends CustomPainter {
  ...
  void _paintSquares(Canvas canvas, Size size) {
    // ScaleFactor is used to scale the background to the whole screen.
    double scaleFactor = 0.1;
    for (int index = 0; index <= size.height / scaleFactor; index += 150) {
      _paintSquaresRow(canvas, size, index);
    }
  }

  void _paintSquaresRow(Canvas canvas, Size size, int squareStartingPoint) {
    final paint = Paint()
